...
struct AppState {
    ...
    pitlane::lite::LocalObsServer obs_server;
...
void obs_log_callback(void* context, std::wstring message) {
    post_log(static_cast<HWND>(context), std::move(message));
}
...
void drain_messages(AppState& state) {
    ...
    while (state.pipeline.try_dequeue(message)) {
        ...
        state.obs_server.broadcast(message);
    }
}
