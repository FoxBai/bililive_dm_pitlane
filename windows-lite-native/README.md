...
- 连接直播间按钮，会用 WinHTTP 访问 B 站真实 API，解析真实房间号、获取弹幕服务器信息，并按 WSS、WS、TCP 顺序尝试实时弹幕连接；连接中再次点击可断开。
...
- B 站 WSS / WS / TCP 弹幕节点循环重试；单个节点断开后会尝试下一个节点，全部断开后 5 秒重连。
...
Lite 版仍没有做到 WPF 版的全部成熟度。B 站接收已有基础 WSS、WS、TCP 连接、WBI 签名、鉴权、心跳、可断开接收循环、压缩包解码、未压缩 JSON 弹幕解析、buvid3 兜底、节点重试和同 UID 脱敏昵称缓存；OBS 本地服务已有基础 HTTP/SSE 链路和浏览器源发车动画预览；更完整的消息解析和历史弹幕补偿还需要继续补。
...
- 人看：当前它已经能打开一个 Win32 测试窗口，能连 B 站基础 WSS / WS / TCP 弹幕，能显示原生透明叠加层，也能启动基础 OBS 本地服务。
